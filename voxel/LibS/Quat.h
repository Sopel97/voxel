...
namespace ls
{
    ...
    template <class T>
    class alignas(detail::quat_align<float>::value) Quat
    {
    public:
        T w;
        T x;
    ...
    constexpr Quat<T> slerp(const Quat<T>& lhs, const Quat<T>& rhs, const T& t)
    {
        ...
        Quat<T> q = rhs;
        T cosTheta = clamp(lhs.dot(rhs), T(-1.0), T(1.0));
